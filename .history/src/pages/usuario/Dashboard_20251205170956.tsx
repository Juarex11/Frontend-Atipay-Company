...
export default function Dashboard() {
  ...
  // Agrega esta línea:
const [pointsHistory, setPointsHistory] = useState<MonthlyHistoryItem[]>([]);
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // Agrega esto:
try {
        const statusData = await getMonthlyStatus();
        if (statusData) setMonthlyStatus(statusData);
      } catch (e) {
        console.error("Error status:", e);
      }

      try {
        const historyData = await getMonthlyHistory();
...
{/* INICIO NUEVO GRÁFICO DE PUNTOS */}
        <div className="mt-6">
           {/* Solo lo mostramos si hay datos */}
           {pointsHistory.length > 0 && (
              <PointsHistoryChart data={pointsHistory} />
           )}
        </div>
        {/* FIN NUEVO GRÁFICO DE PUNTOS */}
