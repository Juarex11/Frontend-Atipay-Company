...
const UsersManagement = () => {
  ...
  const filteredUsers = (users: UserListItem[]) => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      const dateB =
        b.registration_date && b.registration_time
  ...
  const getRoleBadge = (
    ...
    > = {
      admin: {
        ...
        label: "Admin",
        className:
          "bg-[#0a7e3e] hover:bg-[#0c4a2a] text-white border-[#0a7e3e]",
      },
      user: {
        variant: "secondary",
        label: "Socio",
  ...
  const renderUsersTable = () => {
    // Filtramos la respuesta que viene del query
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr className="bg-gradient-to-r from-[#0c4a2a] to-[#0a7e3e]">
                    ...
                    <th className="px-3 py-3 text-left text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap font-bold italic">
                      Estado Calificación
                    </th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap">
                      Puntos Mes
                    ...
                    <th className="px-3 py-3 text-left text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap hidden md:table-cell">
                      Código Ref.
                    </th>
                    <th className="px-3 py-3 text-left text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap hidden xl:table-cell">
                      Fecha Reg.
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-100">
                  {users.map((user: any) => (
                    ...
                    >
                      <td className="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            onClick={(e) => handleConfigureUser(e, user)}
                          >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap">
                        {/* El Badge usa el status dinámico que envía el backend (active/inactive) */}
                        {getStatusBadge(user.status)}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-sm font-bold text-blue-600">
                        {/* Campo current_month_points del backend mapeado como monthly_points */}
                        {Number(user.monthly_points || 0).toLocaleString()} pts
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap">
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-sm text-slate-500 font-medium">
                        {/* Puntos acumulados totales para canje */}
                        {Number(user.accumulated_points || 0).toLocaleString()}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">
                        {user.reference_code || "N/A"}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-400 hidden xl:table-cell">
    ...
    const handleRefresh = (ev?: Event) => {
      ...
      try {
        ...
        if (
          ...
          refetch()
            .then(() => {
              const updated = (usersResponse?.items || []).find(
          ...
          {pagination.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 px-2">
              <div className="text-sm text-muted-foreground whitespace-nowrap">
                Mostrando {usersResponse?.items?.length || 0} de{" "}
                {usersResponse?.total || 0} usuarios
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
