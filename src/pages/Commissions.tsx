...
const Commissions: React.FC = () => {
  ...
  const [history, setHistory] = useState<{
    history: {
      [key: string]: { total: number; comisiones: UnwithdrawnCommission[] };
    };
  } | null>(null);
  ...
  // Valores por defecto (se actualizarán con la DB)
  ...
  const fetchCommissionHistory = async () => {
    try {
      ...
      const message =
        ...
        "Error al cargar el historial de comisiones";
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        const settingsRes = await fetch(`${API_BASE_URL}/commissions/settings`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
        const settingsData = await settingsRes.json();
        
        // ✅ CORRECCIÓN AQUÍ: Usamos el nombre exacto de la columna en la DB 'withdrawal_fee_percent'
        // Si en la DB dice 6, ahora feePercent valdrá 6.
        if (settingsData.withdrawal_fee_percent !== undefined) {
            setFeePercent(Number(settingsData.withdrawal_fee_percent));
        } else if (settingsData.fee_percent !== undefined) {
            // Fallback por si acaso el backend renombra la variable
            setFeePercent(Number(settingsData.fee_percent));
        }

        // Cargamos también los puntos mínimos si existen
        if (settingsData.min_points !== undefined) setMinPoints(Number(settingsData.min_points));
      ...
      const userData = profileRes as any;

      // 2. Detectamos el nombre del mes actual (ej: "feb")
      const currentMonthName = new Date()
        .toLocaleString("es-ES", { month: "short" })
        .replace(".", "")
        .toLowerCase();

      // 3. Buscamos en el historial (que está limpio por mes)
      const personalPoints = userData.points_history?.find((p: any) =>
        p.name.toLowerCase().includes(currentMonthName)
      )?.puntos;

      // 4. Si existe en el historial usamos ese, si no, usamos el total
      setUserPoints(personalPoints !== undefined ? Number(personalPoints) : Number(userData.monthly_points) || 0);
      ...
      if (error.response?.url?.includes("summary")) {
        setError((prev) => ({
          ...prev,
          summary: error.response?.url?.includes("summary")
            ? message
            : prev.summary,
          history: error.response?.url?.includes("history")
            ? message
            : prev.history,
          profile: error.response?.url?.includes("profile")
            ? message
            : prev.profile,
        }));
      }
  ...
  // --- LÓGICA DE BLOQUEO ---
  const hasEnoughPoints = userPoints >= minPoints;
  const hasMoneyPending = (summary?.summary.total_pendientes || 0) > 0;
  
  const isButtonDisabled = loading.withdrawing || !hasMoneyPending || !hasEnoughPoints;
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 text-sm md:text-base">
            Administra y retira tus comisiones generadas
          </p>
        </div>
        ...
        <div className="mt-4 md:mt-0">
          <button
            ...
            className={`w-full md:w-auto px-6 py-3 rounded-lg font-medium text-white shadow-lg transition-all duration-200 transform ${
              isButtonDisabled
          ...
          >
            {loading.withdrawing ? (
              ...
              `Bloqueado (Faltan ${minPoints - userPoints} pts)`
            ) : !hasMoneyPending ? (
          ...
          {!hasEnoughPoints && (
             <p className="mt-2 text-xs text-orange-600 font-medium text-center md:text-right">
                Meta mensual: {minPoints} puntos para desbloquear.
             </p>
          )}
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-green-100 hover:shadow-lg transition-shadow duration-300">
          <div className="p-6">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">Mis puntos (Mes Actual)</p>
                {loading.profile ? (
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <Star className="w-5 h-5 text-orange-500 fill-orange-500 ml-1" />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                {loading.summary ? (
                  ...
                  <div className="flex items-center space-x-1">
                    <p className="text-2xl font-bold text-gray-800">
                      {summary?.summary.total_retiradas.toFixed(2)}
                    </p>
          ...
          <div className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                {/* ✅ AQUÍ ESTÁ EL MENSAJE DINÁMICO QUE USA feePercent (ahora será 6) */}
                <p className="text-xs text-green-100 mt-1">
                  {userPoints >= minPoints 
                    ? `¡Tienes ${userPoints.toLocaleString("es-ES")} puntos! Comisión del ${feePercent}% aplicada.` 
                    : `Necesitas ${(minPoints - userPoints)} puntos más para poder retirar`}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Visualiza todas tus comisiones con su estado actual
          </p>
        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <span className="text-sm font-medium text-gray-900">
                          {commission.puntos_generados.toString()}
                        </span>
                      ...
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-1.5">
                            <AtipayCoin size="xs" className="text-yellow-500" />
                            <span className="text-sm font-medium text-gray-900">
                              {parseFloat(item.amount).toLocaleString("es-ES", {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </span>
