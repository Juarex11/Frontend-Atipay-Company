...
import { 
    Search, Package, Plus, X, Edit, Trash2, Layers, 
    Save, Camera, Upload, Eye, EyeOff, CheckSquare, 
    AlertCircle, CheckCircle, ChevronLeft, ChevronRight 
} from 'lucide-react';
import type { Product, Pack } from '../../../services/adminPurchaseService';
import { createProduct, updateProduct, deleteProduct, deletePack } from '../../../services/adminPurchaseService';
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-sm w-full border border-gray-100 transform transition-all animate-in zoom-in-95">
            <div className="flex flex-col items-center text-center space-y-4">
                <div className={`p-4 rounded-full ${type === 'error' ? 'bg-red-50 text-red-500' : type === 'success' ? 'bg-green-50 text-green-500' : 'bg-amber-50 text-amber-500'}`}>
...
export const CatalogPanel = ({ 
    products, packs, loading, 
    onAddProduct, onAddPack, onSaveCartAsPack, onRulesChange, onEditPack,
    isDeleteMode, setIsDeleteMode, selectedToDelete, onExecuteBulkDelete, onCancelDeleteMode
}: CatalogPanelProps) => {
    ...
    const isSelected = (id: number, type: 'product' | 'pack') => {
        return selectedToDelete.some(item => item.id === id && item.type === type);
    };

    // --- HANDLERS ---

    const handleSaveProduct = async () => {
        if (!prodName.trim() || !prodPrice || !prodPoints) return showAlert('warning', 'Faltan Datos', 'Nombre, Precio y Puntos requeridos.');
        if (Number(prodPrice) < 0 || Number(prodPoints) < 0) return showAlert('error', 'Valor Invalido', 'No negativos.');
        setIsCreating(true);
        try {
            await createProduct({ name: prodName, price: parseFloat(prodPrice), points: parseFloat(prodPoints), stock: 100, type: 'product', description: 'Producto Rapido', image: prodImage, is_visible: prodVisible });
            ...
            setShowQuickCreate(false); onRulesChange(); showAlert('success', 'Creado', 'Producto agregado con exito.');
        } catch (error) { console.error(error); const err = error as any; showAlert('error', 'Error', err.message || 'Error al crear.'); } finally { setIsCreating(false); }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            onRulesChange(); 
            showAlert('success', 'Eliminado', 'El item ha sido eliminado correctamente.');
        } catch (error: any) {
            console.error("Error al eliminar:", error);
            const errorMsg = error.message || '';
            if (errorMsg.includes('404') || errorMsg.includes('found') || errorMsg.includes('encontrado')) {
                onRulesChange();
                showAlert('warning', 'Aviso', 'El item ya no existia, se ha actualizado la lista.');
            } else {
    ...
    const saveEdit = async (id: number) => {
        ...
        try {
            ...
            setEditingId(null); setEditImage(null); onRulesChange(); showAlert('success', 'Actualizado', 'Producto editado correctamente.');
    ...
    return (
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 h-full flex flex-col overflow-hidden relative">
            {alertState && <CustomAlert type={alertState.type} title={alertState.title} message={alertState.msg} onClose={() => setAlertState(null)} />}
            {confirmState && <ConfirmModal title="¿Eliminar Item?" message="¿Estás seguro de que deseas eliminar este item permanentemente?" onConfirm={confirmDelete} onCancel={() => setConfirmState(null)} />}

            {/* HEADER */}
            <div className="flex border-b border-gray-100 shrink-0 bg-white relative z-20">
                <button onClick={() => setActiveTab('catalog')} className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'catalog' ? 'text-green-600 border-b-2 border-green-500 bg-green-50/50' : 'text-gray-400 hover:text-gray-600'}`}><Package className="w-4 h-4"/> Catalogo</button>
            ...
            <div className="p-5 flex-1 overflow-y-auto custom-scrollbar space-y-6 pb-5"> 
                ...
                {activeTab === 'catalog' && (
                    <>
                        {!isDeleteMode && (
                            <div className="bg-orange-50/50 rounded-2xl p-4 border border-orange-100">
                                <div className="flex justify-between items-center mb-3 cursor-pointer" onClick={() => setShowQuickCreate(!showQuickCreate)}>
                                    <h4 className="text-xs font-bold text-orange-600 uppercase flex items-center gap-2"><Plus className="w-3 h-3"/> Creacion Rapida</h4>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-2">
                            {loading ? <p className="text-center text-gray-400 col-span-full py-4">Cargando catalogo...</p> : 
                             // @ts-ignore
                             paginatedItems.map((product: Product) => {
                                ...
                                <div 
                                    ...
                                    className={`group bg-white border p-3 rounded-2xl transition-all flex flex-col justify-between relative overflow-hidden
                                        ${isDeleteMode 
                                            ? (selected ? 'border-red-500 ring-2 ring-red-100 bg-red-50 cursor-pointer' : 'border-gray-200 hover:border-red-300 cursor-pointer') 
                                ...
                                >
                                    {isDeleteMode && (
                                        <div className={`absolute top-2 right-2 z-10 p-1 rounded-full bg-white shadow-sm transition-transform ${selected ? 'scale-110' : 'scale-100'}`}>
                                            {selected ? <CheckCircle className="w-5 h-5 text-red-600 fill-red-100" /> : <div className="w-5 h-5 rounded-full border-2 border-gray-300" />}
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className={`w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center shrink-0 overflow-hidden relative ${!isDeleteMode ? 'cursor-pointer' : ''} ${editingId === product.id ? 'ring-2 ring-green-400' : ''}`} onClick={() => !isDeleteMode && editingId === product.id && editFileInputRef.current?.click()}>
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-50">
                                        ...
                                        {!isDeleteMode && (
                                            <div className="flex items-center gap-1">
                                                <button onClick={() => startEdit(product)} className="p-1.5 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Edit className="w-3.5 h-3.5"/></button>
                                                <button onClick={() => requestDelete(product.id, 'product')} className="p-1.5 text-red-300 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
                                                <button onClick={() => onAddProduct(product)} className="w-8 h-8 rounded-full bg-green-50 text-green-600 flex items-center justify-center hover:bg-green-500 hover:text-white transition-colors ml-1"><Plus className="w-4 h-4"/></button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                    ...
                    <div className="space-y-4">
                        {!isDeleteMode && (
                            <div className="flex justify-between items-center bg-purple-50 p-4 rounded-xl border border-purple-100">
                                <div><h3 className="text-sm font-bold text-purple-900">Gestion de Packs</h3><p className="text-xs text-purple-600 hidden sm:block">Crea packs con logica.</p></div>
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4 w-full">
                                        <div className="w-16 h-16 bg-purple-50 rounded-xl flex items-center justify-center text-purple-400 shrink-0 overflow-hidden border border-purple-100 relative">
                                            {packImg ? (
                                                <img src={packImg} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-500" alt={pack.name} onError={(e) => {(e.target as HTMLImageElement).style.display = 'none'; (e.target as HTMLImageElement).nextElementSibling?.classList.remove('hidden');}} />
                                            ) : null}
                                            <Layers className={`w-8 h-8 text-purple-300 ${packImg ? 'hidden' : ''}`} />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <h4 className="font-bold text-gray-900 text-sm truncate">{pack.name}</h4>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[10px] font-bold bg-gray-100 text-gray-600 px-2 py-0.5 rounded border border-gray-200">S/ {Number(pack.total_pack_price).toFixed(2)}</span>
                                                <span className="text-[10px] font-bold bg-purple-50 text-purple-600 px-2 py-0.5 rounded border border-purple-100">{Number(pack.total_pack_points).toFixed(2)} pts</span>
                                            </div>
                                            <p className="text-[10px] text-gray-400 mt-1 truncate">{pack.products && pack.products.length > 0 ? `${pack.products.length} productos incluidos` : 'Sin productos'}</p>
                                        </div>
                                    </div>
                                    ...
                                    {!isDeleteMode && (
                                        <div className="flex items-center gap-1 mt-0 shrink-0 ml-2">
                                            <div className="flex items-center bg-gray-50 rounded-lg p-1 mr-2 border border-gray-100">
                                                <button onClick={() => onEditPack(pack)} className="p-1.5 text-purple-400 hover:text-purple-600 hover:bg-white rounded-md transition-colors"><Edit className="w-4 h-4"/></button>
                                                <button onClick={() => requestDelete(pack.id, 'pack')} className="p-1.5 text-red-300 hover:text-red-500 hover:bg-white rounded-md transition-colors"><Trash2 className="w-4 h-4"/></button>
                                            </div>
                                            <button onClick={() => onAddPack(pack)} className="w-10 h-10 bg-purple-600 text-white rounded-xl flex items-center justify-center hover:bg-purple-700 shadow-md hover:shadow-purple-200 transition-all active:scale-95"><Plus className="w-5 h-5"/></button>
                                        </div>
                                    )}
                                </div>
                            )})}
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center items-center gap-4 mt-4 py-2">
                        <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-lg bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"><ChevronLeft className="w-5 h-5"/></button>
                        <span className="text-xs font-bold text-gray-500">Pagina {currentPage} de {totalPages}</span>
                        <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"><ChevronRight className="w-5 h-5"/></button>
                    </div>
                )}
            </div>
        </div>
