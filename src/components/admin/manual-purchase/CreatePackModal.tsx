...
// --- 1. COMPONENTES VISUALES PARA ALERTAS (Bonitos) ---
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-sm w-full border border-gray-100 transform transition-all animate-in zoom-in-95">
            <div className="flex flex-col items-center text-center space-y-4">
                ...
                <button onClick={onClose} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition-transform active:scale-95 ${type === 'error' ? 'bg-red-500 hover:bg-red-600' : type === 'success' ? 'bg-green-500 hover:bg-green-600' : 'bg-amber-500 hover:bg-amber-600'}`}>
                    Entendido
                </button>
            </div>
        </div>
...
const ConfirmationModal = ({ title, message, onConfirm, onCancel }: { title: string, message: string, onConfirm: () => void, onCancel: () => void }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-sm w-full border border-gray-100 transform transition-all animate-in zoom-in-95">
            <div className="flex flex-col items-center text-center space-y-4">
                <div className="p-4 rounded-full bg-blue-50 text-blue-500"><AlertCircle className="w-8 h-8" /></div>
                <div>
                    <h3 className="text-lg font-extrabold text-gray-900">{title}</h3>
                    <p className="text-sm text-gray-500 mt-2 leading-relaxed">{message}</p>
                </div>
                <div className="flex gap-3 w-full">
                    ...
                    <button onClick={onConfirm} className="flex-1 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition-transform active:scale-95">Continuar</button>
                </div>
            </div>
        </div>
    </div>
...
export const CreatePackModal: React.FC<CreatePackModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    // Alertas y Confirmaciones (NUEVO)
    ...
    const handleAddCustomItem = () => {
        ...
        if (parseFloat(newItemPrice) < 0 || parseFloat(newItemPoints) < 0) return showAlert('error', 'Error', "❌ No se permiten negativos.");
    ...
    // --- LÓGICA DE GUARDADO REFACTORIZADA (Sin alerts nativos) ---
    
    // 1. Iniciador del proceso
    const handleSaveClick = () => {
        if (!packName.trim()) return showAlert('warning', 'Nombre requerido', "Falta el nombre del Pack");
        if ((selectedIds.length === 0 && customItems.length === 0)) return showAlert('warning', 'Pack vacío', "Agrega al menos un producto o item.");

        // Verificación 1: Balance de puntos
        if (!isBalanced) {
            setConfirmState({
                show: true,
                title: 'Puntos no coinciden',
                msg: `Asignado: ${totalAssignedPoints.toFixed(2)} vs Meta: ${targetTotalPoints.toFixed(2)}. ¿Guardar de todos modos?`,
                action: checkExistingProducts // Si acepta, pasa a la siguiente verificación
            });
            return;
        }

        // Si está balanceado, pasamos directo a la siguiente verificación
        checkExistingProducts();
    };

    // 2. Verificación de productos existentes
    const checkExistingProducts = () => {
        if (selectedIds.length > 0) {
            setConfirmState({
                show: true,
                title: 'Actualizar Productos',
                msg: "⚠️ Se actualizarán los puntos de los productos existentes seleccionados en el catálogo general. ¿Continuar?",
                action: executeSave // Si acepta, guarda
            });
            return;
        }
        executeSave(); // Si no hay productos existentes, guarda directo
    };

    // 3. Ejecución final del guardado
    const executeSave = async () => {
        setConfirmState(null); // Cerrar confirmaciones
        setLoading(true);
        try {
            ...
            showAlert('error', 'Error al guardar', err.message || "Error desconocido");
        } finally { 
            setLoading(false); 
        }
    };
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
            ...
            <div className="bg-white rounded-3xl w-full max-w-5xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden border border-gray-100 transform transition-all animate-in zoom-in-95">
                ...
                <div className="p-5 border-t border-gray-100 bg-white flex justify-between items-center shrink-0">
                    ...
                    {step === 1 ? (
                        <div className="flex justify-end w-full"><button onClick={() => (selectedIds.length > 0 || customItems.length > 0) ? setStep(2) : showAlert('warning', 'Atención', "Selecciona al menos 1 producto")} className="bg-gray-900 text-white px-8 py-2.5 rounded-xl font-bold text-sm hover:bg-black shadow-lg transition-transform active:scale-95 flex items-center gap-2">Siguiente <ArrowRight className="w-4 h-4"/></button></div>
                    ) : (
