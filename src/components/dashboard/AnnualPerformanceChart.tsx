          ...
          >
            ...
            >
              {data.map((entry, index) => {
                // ✅ Lógica: Verde fuerte si llega a 100, naranja si tiene algo, gris si es 0
                const barColor =
                  entry.points >= 100
                    ? "#10B981" // Verde (Meta cumplida)
                    : entry.points > 0
                      ? "#FBBF24" // Naranja (En progreso)
                      : "#F3F4F6"; // Gris (Sin puntos)

                return (
                  <Cell key={`cell-${index}`} fill={barColor} fillOpacity={1} />
                );
              })}
            </Bar>
          </BarChart>
