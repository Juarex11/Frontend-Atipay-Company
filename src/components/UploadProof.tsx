...
export const UploadProof: React.FC<UploadProofProps> = ({ purchase, onClose, onSuccess }) => {
  ...
  const handleFileSelect = (selectedFile: File | null) => {
    ...
    // CAMBIO: Límite aumentado a 10MB
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden">
        <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-200/70">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="p-3 text-xs bg-gray-50 rounded-lg border border-gray-200 flex gap-2">
            ...
            <div className="space-y-1">
              ...
              <ul className="list-disc pl-5 space-y-0.5 text-gray-600">
                ...
                {/* CAMBIO: Texto actualizado a 10MB */}
                <li>Tamaño máximo: <b>10MB</b>.</li>
          ...
          {!file ? (
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${isDragging ? 'bg-green-50 border-green-500 scale-[0.98]' : 'border-gray-300 hover:border-green-400 hover:bg-gray-50'}`}
            >
              ...
              {/* CAMBIO: Texto actualizado a 10MB */}
              <p className="text-xs text-gray-500 mt-1">Solo imágenes de hasta 10MB</p>
              <input id="file-upload" type="file" className="hidden" accept="image/*" onChange={(e) => handleFileSelect(e.target.files?.[0] || null)} />
            </div>
          ) : (
            <div className="relative border rounded-xl overflow-hidden shadow-md group">
              {preview && <img src={preview} className="w-full h-56 object-cover" alt="Preview" />}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                <Button variant="destructive" size="sm" onClick={removeFile} className="rounded-full">
          ...
          {proofUrl && !file && (
            <div className="border border-green-200 bg-green-50 p-3 rounded-lg flex justify-between items-center">
              ...
              <a href={proofUrl} target="_blank" rel="noreferrer" className="text-xs bg-white px-3 py-1.5 rounded-md border shadow-sm hover:text-green-900 flex items-center gap-1">
        ...
        <div className="p-4 border-t bg-gray-50 flex justify-end gap-2">
          ...
          <Button onClick={handleUpload} disabled={!file || loading} className="bg-green-600 hover:bg-green-700 text-white min-w-[140px]">
            {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Enviando...</> : <>Enviar comprobante</>}
          </Button>
        </div>
      </div>
    </div>
